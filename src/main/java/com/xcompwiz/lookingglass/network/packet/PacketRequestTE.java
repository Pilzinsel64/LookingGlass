...
public class PacketRequestTE extends PacketHandlerBase {
    ...
    public void handle(ByteBuf data, EntityPlayer player) {
        ...
        if (tile != null) {
            // FIXME: This is currently a very "forceful" method of doing this, and not technically guaranteed to
            // produce correct results
            // This would be much better handled by using the getDescriptionPacket method and wrapping that packet in a
            // LookingGlass
            // packet to control delivery timing, allowing for processing the packet while the correct target world is
            // the active world
            ...
            ServerPacketDispatcher.getInstance()
                .addPacket(player, PacketTileEntityNBT.createPacket(xPos, yPos, zPos, tag, dim));
        }
    }
}
