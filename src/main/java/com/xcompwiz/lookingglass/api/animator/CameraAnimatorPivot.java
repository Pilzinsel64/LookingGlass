...
/**
 * This is a standard sample implementation of a camera animator. It simply uses the target location as a LookAt target
 * and does a fly pivot around it. It can
...
public class CameraAnimatorPivot implements ICameraAnimator {
    ...
    /**
     * This is a list of recommended, preset offsets that the animator will choose from. It's use can be overridden via
     * function.
     */
    private static final int[][] presets = { { 2, 5 }, { 5, 9 }, { 9, 15 }, { 1, 3 }, { 2, 1 }, { 0, 2 } };
    /**
     * This is a pair used when the animator cannot find a good path from the presets. It's use can be overridden via
     * function.
     */
        ...
        for (int j = -distance; j <= distance; ++j) {
            for (int k = -distance; k <= distance; ++k) {
                if (!camera.getBlockData()
                    .isAirBlock(xCenter + j, yCenter + up, zCenter + k)) return false;
            }
        }
        return true;
    }
