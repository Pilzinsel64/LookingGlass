...
/**
 * The purpose of this interface is simply to provide instances of the API interfaces. Request an instance via IMC (see
 * below). This interface supports
 * providing multiple versions of an API, so you can request ex. 'symbol-1' and always get the same interface, enabling
 * the API to move forward without breaking
 * compatibility. The provider instance provided to you, as well as any API interface instances created by it, will
 * belong to the mod which requested the
 * provider.
 * 
 * @author xcompwiz
 */
public interface APIInstanceProvider {
    ...
    /**
     * Returns a constructed version of the requested API. If the API requested doesn't exist then an exception will be
     * thrown. Be wary when attempting to cast
     * the returned instance, as, if you try using an interface not included in the class path, you may get missing
     * definition crashes. It is wiser to, after
     ...
     * @return The requested API instance as an Object. If you get an object, it is guaranteed to be of the requested
     *         API and version.
     */
