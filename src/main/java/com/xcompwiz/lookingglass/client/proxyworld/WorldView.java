...
public class WorldView implements IWorldView {
    ...
    public WorldView(WorldClient worldObj, ChunkCoordinates coords, int width, int height) {
        ...
        this.effectRenderer = new EffectRenderer(
            ...
            Minecraft.getMinecraft()
                .getTextureManager());
        // Technically speaking, this is poor practice as it leaks a reference to the view before it's done
        // constructing.
        this.fbo = FrameBufferContainer.createNewFramebuffer(this, width, height);
    }

    /**
     * Explicitly shuts down the view. Informs the frame buffer manager that we don't want our framebuffer anymore (so
     * it can be cleaned up for certain on the
    ...
    /**
     * This is a really complex bit. As we want to reuse the current client world when rendering, if possible, we need
     * to handle when that world changes. We
     * could simply destroy all the views pointing to the existing proxy world, but that would be annoying to mods using
     * the API. Instead, we replace our proxy
     * world with the new client world. This should only be called by LookingGlass, and only from the handling of the
     * client world change detection.
     * 
