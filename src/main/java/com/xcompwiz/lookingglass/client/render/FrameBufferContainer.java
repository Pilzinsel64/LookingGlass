...
public class FrameBufferContainer {
    ...
    /**
     * Using this map we can detect which FBOs should be freed. The map will delete any entry where the world view
     * object is garbage collected. It unfortunately
     * can't detect that the world view is otherwise leaked, though, just when it's gone.
     */
    private static ConcurrentMap<WorldView, FrameBufferContainer> weakfbomap = new MapMaker().weakKeys()
        .<WorldView, FrameBufferContainer>makeMap();
    ...
    private void allocateFrameBuffer() {
        ...
        this.framebuffer = EXTFramebufferObject.glGenFramebuffersEXT(); // Release via:
                                                                        // EXTFramebufferObject.glDeleteFramebuffersEXT(framebuffer);
        this.depthBuffer = EXTFramebufferObject.glGenRenderbuffersEXT(); // Release via:
        ...
        if (MinecraftForgeClient.getStencilBits() == 0) EXTFramebufferObject.glRenderbufferStorageEXT(
            EXTFramebufferObject.GL_RENDERBUFFER_EXT,
            GL14.GL_DEPTH_COMPONENT24,
            width,
            height);
        else EXTFramebufferObject.glRenderbufferStorageEXT(
            EXTFramebufferObject.GL_RENDERBUFFER_EXT,
        ...
        EXTFramebufferObject.glFramebufferRenderbufferEXT(
            ...
            EXTFramebufferObject.GL_RENDERBUFFER_EXT,
            depthBuffer);
        if (MinecraftForgeClient.getStencilBits() != 0) EXTFramebufferObject.glFramebufferRenderbufferEXT(
            EXTFramebufferObject.GL_FRAMEBUFFER_EXT,
